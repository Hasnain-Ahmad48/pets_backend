...
const addOrUpdateOrderReview = async (req, res) => {
  try {
    ...
    if (!order_id || !user_id || !rating) {
      return res
        .status(400)
        .json({message: "order_id, user_id, rating are required"});
    }
    ...
    if (!verify) {
      return res.status(400).json({
        message:
          "Order not found, not delivered, or does not belong to this user",
      });
    }

    // Step 2: Insert or Update review
    const result = await reviewModel.addOrUpdateReview(
      order_id,
      user_id,
      rating,
      review,
...
const addOrUpdateProductReview = async (req, res) => {
  try {
    ...
    if (!order_id || !product_id || !user_id || !rating) {
      return res
        .status(400)
        .json({message: "order_id,product_id, user_id, rating are required"});
    }
    ...
    // Step 2: Insert or Update review
    const result = await reviewModel.addOrUpdateProductReview(
      order_id,
      user_id,
      product_id,
      rating,
...
const getAllFilterPriceProductsController = async (req, res) => {
  ...
  try {
    const result = await mkProductModel.getFilterPriceProducts(
      fromPrice,
      toPrice,
      slug,
    );
...
const updateProduct = async (req, res) => {
  try {
    ...
    let slug = baseSlug;
    let slugExists = await mkProductModel.checkSlugExists(slug, id);
    let slugCounter = 1;

    while (slugExists) {
      slug = `${baseSlug}-${slugCounter}`;
      slugExists = await mkProductModel.checkSlugExists(slug, id);
      slugCounter++;
    }
...
const createPaymentIntent = async (req, res, next) => {
  ...
  const user_id = req.user.id;
  const {shipping_cost = 0, paymentMethodType, currency, items} = req.body;
...
const createDiscountController = async (req, res) => {
  ...
  try {
    const result = await mkProductModel.createDiscount(
      shop_id,
      name,
      discount,
      isPublish,
