...
// Decrypt the received password
 ...
 return authUserModel.updateUserAuthToken(user.id, authToken, firebase_token, deviceType).then((data) => {
...
exports.signinWithGoogle = async (req, res) => {
  ...
  const client = new OAuth2Client('876551009681-82kj7i7vq8hfcgji93qqfgcmv6lrr3ja.apps.googleusercontent.com');

  try {
    const ticket = await client.verifyIdToken({
      idToken: token,
      audience: '876551009681-82kj7i7vq8hfcgji93qqfgcmv6lrr3ja.apps.googleusercontent.com',
    });
    const payload = ticket.getPayload();
    
    // Check if the user exists
    authUserModel.loginUser_(payload.email)
      .then(user => {
        ...
        authUserModel.updateUserAuthToken(user.id, authToken, firebase_token, deviceType)
          .then(() => {
            res.json({
          ...
          .catch(error => {
            res.status(500).json({ message: 'Error updating user token', error: error.message });
          });
      ...
      .catch(async (error) => {
        if (error.message === 'User not found') {
          ...
          try {
            const newUser = await authUserModel.signupWithGoogle(payload.given_name, payload.family_name, payload.email, payload.picture,payload.sub);
            ...
            authUserModel.updateUserAuthToken(newUser.id, authToken, firebase_token, deviceType)
              .then(() => {
                res.json({
              ...
              .catch(error => {
                res.status(500).json({ message: 'Error updating token for new user', error: error.message });
              });
          } catch (signupErr) {
            res.status(500).json({ message: 'Error signing up new user', error: signupErr.message });
          }
        } else {
...
// Decrypt the received password
  ...
  authUserModel
    ...
    .then((user) => {
      ...
      return authUserModel.updateUserAuthToken(id, authToken, firebase_token, deviceType).then((data) => {
        res.json({
          message: "Login successful",
          data: {
            //  id: user.id,
...
exports.getTrackingData = async (req, res) => {
  try {
    ...
    const rows = await authUserModel.getTrackingData(trackerId, startDate, startTime, endDate, endTime);
